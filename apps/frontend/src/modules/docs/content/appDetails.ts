        ...
        blocks: [
          {
            type: "bullets",
            items: [
              "Effect 1 — fires on any selection change ([isGenerated, instrument, timeframe, customRange, retryKey, generateKey, expiryDate, callStrike, putStrike, type]): checks /api/market/status, fetches Future/CE/PE/Spot OHLC in parallel plus a 200-bar Spot warm-up series, builds every historical row synchronously, seeds EMA/EMA200/VWAP continuation state (prevEmaRef, prevEma200Ref, vwapStateRef) and session high/low (swHighRef/swLowRef).",
